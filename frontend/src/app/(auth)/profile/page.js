...
// Mock data tài liệu đã đăng (Sau này sẽ fetch từ API theo userId)
const mockUserDocs = [
  { id: 1, title: "Đồ án ReactJS - Hệ thống Quản lý Thư viện số", type: "Source Code", status: "Approved", views: 1205, date: "12/05/2026" },
  { id: 5, title: "Tài liệu ôn tập Cấu trúc dữ liệu & Giải thuật", type: "PDF", status: "Approved", views: 450, date: "10/05/2026" },
  { id: 6, title: "Slide thuyết trình: An toàn bảo mật thông tin", type: "Slide", status: "Pending", views: 0, date: "15/05/2026" },
];
...
export default function ProfilePage() {
  ...
  // Nếu chưa login thì hiển thị nhắc nhở (hoặc redirect)
  if (!session) {
    return (
        ...
        <div className="bg-white rounded-[3rem] border border-slate-100 shadow-[0_30px_60px_-15px_rgba(0,0,0,0.03)] overflow-hidden mb-12 relative group">
           ...
           <div className="px-10 md:px-16 pb-12 relative">
              <div className="flex flex-col md:flex-row items-end gap-8 -mt-16 mb-8">
                 <div className="relative group/avatar">
                    <div className="w-32 h-32 md:w-40 md:h-40 rounded-[3rem] bg-white p-2 shadow-2xl relative z-10 overflow-hidden">
                       <div className="w-full h-full rounded-[2.5rem] bg-gradient-to-br from-emerald-400 to-blue-500 flex items-center justify-center text-white text-5xl font-black shadow-inner">
                 ...
                 <div className="flex-1 space-y-2 pb-2">
                    <div className="flex items-center gap-3">
                       <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tighter uppercase italic">{session.user.name || "Người dùng DataHub"}</h1>
                       <div className="px-3 py-1 bg-emerald-100 text-emerald-600 rounded-lg text-[9px] font-black uppercase tracking-widest shadow-sm">Verified</div>
                    </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 pt-8 border-t border-slate-50">
                 {[
                   { label: "Tài liệu đã đăng", value: "12", color: "text-emerald-500" },
                   { label: "Tổng lượt xem", value: "2.4k", color: "text-blue-500" },
                   { label: "Lượt tải về", value: "850", color: "text-amber-500" },
                   { label: "Điểm uy tín", value: "9.8", color: "text-purple-500" }
                 ].map((stat, i) => (
        ...
        <div className="grid grid-cols-1 gap-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
           {activeTab === 'uploads' && (
             <div className="space-y-6">
                {mockUserDocs.map((doc) => (
                  <div key={doc.id} className="bg-white rounded-[2.5rem] p-8 border border-slate-100 shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 transition-all group flex flex-col md:flex-row items-center justify-between gap-8">
                     <div className="flex items-center gap-6 flex-1">
                        <div className={`w-16 h-16 rounded-3xl flex items-center justify-center shrink-0 shadow-inner transition-transform group-hover:rotate-6 ${doc.type === 'Source Code' ? 'bg-slate-900 text-white' : 'bg-red-50 text-red-500'}`}>
