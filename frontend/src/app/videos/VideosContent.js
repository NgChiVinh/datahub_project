...
export default function VideosContent() {
  ...
  const [pagination, setPagination] = useState({ currentPage: 1, totalPages: 1, totalMaterials: 0 });
  ...
  const handleAddToCollection = (e, id) => {
    ...
    setSelectedVideoId(id); setIsCollectionModalOpen(true);
  };
  ...
  const logClick = async (materialId) => {
    ...
    try {
      ...
      await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/search-logs`, {
        ...
        body: JSON.stringify({ searchQuery: searchQuery.trim(), clickedMaterialId: materialId }),
      });
  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/materials?${params}`);
        const data = await res.json();
        if (data?.materials) { setVideos(data.materials); setPagination(data.pagination); }
      } catch (err) { console.error(err); }
  ...
  useEffect(() => { setCurrentPage(1); }, [activeMajor, searchQuery, sortBy]);

  const resetFilters = () => { setActiveMajor("all"); setSearchQuery(""); };
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-slate-900 pb-20">
      ...
      {/* Header — mirror DocumentsContent */}
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              ...
              <h1 className="text-3xl font-black text-slate-900 tracking-tight">
                Thư viện <span className="text-orange-500">Video</span>
              </h1>
            </div>
            <Link href="/upload"
              className="inline-flex items-center gap-2 px-6 py-3.5 bg-orange-500 text-white rounded-2xl text-xs font-bold hover:bg-orange-600 transition-all active:scale-95 shadow-lg shadow-orange-100">
      ...
      <div className="container mx-auto max-w-7xl px-4 mt-12">
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          {/* Sidebar — mirror DocumentsContent */}
            ...
            <div className="sticky top-28 space-y-10">
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  {hasFilters && (
                    <button onClick={resetFilters} className="text-[10px] font-bold text-orange-500 hover:underline">Xóa lọc</button>
                  )}
                </div>
              ...
              <div className="bg-slate-900 p-8 rounded-[2rem] text-white">
                <div className="w-10 h-10 bg-orange-100/20 rounded-xl flex items-center justify-center mb-4">
                  <svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5" className="text-orange-400">
                ...
                <p className="text-slate-400 text-[10px] leading-relaxed font-medium">
                  Video bài giảng giúp bạn tiếp thu kiến thức nhanh hơn 40% so với đọc tài liệu thông thường.
                </p>
              </div>
            </div>
          </aside>
          ...
          <main className="flex-1 space-y-8">
            ...
            <div className="flex flex-col md:flex-row items-stretch gap-4">
              <div className="relative flex-1 group">
                ...
                <input type="text" placeholder="Tìm kiếm bài giảng video..."
            ...
            {/* Results count */}
            {!isLoading && pagination.totalMaterials > 0 && (
              <p className="text-[11px] font-bold text-slate-400">
                {pagination.totalMaterials.toLocaleString("vi-VN")} video{hasFilters ? " phù hợp" : " trong thư viện"}
              </p>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {isLoading ? (
                ...
                videos.map((video) => {
                  const thumb = getYoutubeThumbnail(video.fileUrl, "maxresdefault")
                    || "https://images.unsplash.com/photo-1516321318423-f06f85e504b3?w=800&q=80";
                  return (
                    <div key={video._id}
                      className="group bg-white rounded-[1.5rem] border border-slate-100 hover:border-orange-500/20 hover:shadow-xl hover:shadow-orange-500/10 hover:-translate-y-1 transition-all duration-300 flex flex-col h-full overflow-hidden">

                      {/* Thumbnail */}
                      <Link href={`/videos/${video._id}`} onClick={() => logClick(video._id)}
                        className="block relative aspect-video overflow-hidden bg-slate-900">
                        <Image src={thumb} alt={video.title} fill
                          className="object-cover transition-transform duration-500 group-hover:scale-[1.04] opacity-90" />
                      ...
                      <div className="p-5 flex flex-col flex-1">
                        <p className="text-[9px] font-black text-orange-500 uppercase tracking-widest mb-2 truncate">
                          {video.categoryId?.name || "Bài giảng"}
                        </p>
                        <Link href={`/videos/${video._id}`} onClick={() => logClick(video._id)}>
                          <h3 className="text-sm font-bold text-slate-800 leading-snug line-clamp-2 hover:text-orange-500 transition-colors mb-4">
                            {video.title}
                          </h3>
                        </Link>
                        ...
                        <div className="flex justify-between items-center pt-4 border-t border-slate-50 mt-auto">
                          ...
                          <div className="flex items-center gap-3">
                            <button onClick={(e) => handleAddToCollection(e, video._id)}
                              className="p-2 text-slate-300 hover:text-orange-500 transition-colors"
                              title="Thêm vào bộ sưu tập">
                              <svg width="18" height="18" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
                                <path strokeLinecap="round" strokeLinejoin="round" d="M12 4L12 20M20 12L4 12" />
                              </svg>
                            </button>
            ...
            {pagination.totalPages > 1 && (
              <div className="flex items-center justify-center gap-2 pt-10">
                ...
                {getPageRange(currentPage, pagination.totalPages).map((p, i) =>
                  p === "..." ? (
                    <span key={`gap-${i}`} className="w-10 h-10 flex items-center justify-center text-slate-300 font-bold">…</span>
                  ) : (
                    <button key={p} onClick={() => setCurrentPage(p)}
                      aria-label={`Trang ${p}`} aria-current={currentPage === p ? "page" : undefined}
                      className={`w-10 h-10 rounded-xl text-xs font-bold transition-all ${
                        currentPage === p ? "bg-slate-900 text-white shadow-lg" : "bg-white border border-slate-200 text-slate-400 hover:border-slate-400 hover:text-slate-600"
                      }`}>
                      {p}
                    </button>
                  )
      ...
      <AddToCollectionModal isOpen={isCollectionModalOpen} onClose={() => setIsCollectionModalOpen(false)} materialId={selectedVideoId} />
    </div>
  );
}
