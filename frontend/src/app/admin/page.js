...
export default function AdminDashboard() {
  ...
  const doughnutData = {
    ...
    datasets: [{
      ...
      backgroundColor: ["#059669","#3b82f6","#f59e0b","#ef4444","#8b5cf6","#ec4899","#64748b"],
      borderWidth: 0,
      hoverOffset: 10,
    }],
  };
  ...
  const doughnutOptions = {
    cutout: "72%",
    plugins: {
      legend: { position: "bottom", labels: { usePointStyle: true, padding: 20, font: { size: 11, weight: "600" }, color: "#64748b" } },
        ...
        <div className="lg:col-span-2 bg-white p-7 rounded-2xl shadow-sm border border-slate-100 flex flex-col">
          <div className="flex justify-between items-start mb-6">
            ...
            <div className="text-right">
              <span className="text-2xl font-bold text-emerald-600">{stats?.summary?.approvedMaterials || 0}</span>
...
function StatCard({ title, value, icon, color, alert }) {
  const colorMap = {
    blue:    { icon: "text-blue-500 bg-blue-50 border-blue-100/50",       ring: "ring-blue-500/5" },
    emerald: { icon: "text-emerald-500 bg-emerald-50 border-emerald-100/50", ring: "ring-emerald-500/5" },
    amber:   { icon: "text-amber-500 bg-amber-50 border-amber-100/50",     ring: "ring-amber-500/5" },
    violet:  { icon: "text-violet-500 bg-violet-50 border-violet-100/50",   ring: "ring-violet-500/5" },
  };
