...
export default function UploadPage() {
  ...
  // Lấy 8 tag đầu tiên từ database làm gợi ý
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 201 || res.status === 200) {
        toast.success("Tải lên thành công! Đang chờ kiểm duyệt.", {
          id: loadingToast,
        });
        ...
        <div className="container mx-auto max-w-6xl px-4 pt-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
            <div>
              <Link
                href="/documents"
                className="inline-flex items-center gap-2 text-emerald-600 font-black text-[10px] uppercase tracking-[0.2em] mb-3 hover:gap-3 transition-all"
              >
                <svg
                  width="14"
                  height="14"
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl w-fit border border-slate-200/50">
              {["file", "link"].map((type) => (
                <button
                  ...
                  className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${uploadType === type ? "bg-white text-slate-900 shadow-sm" : "text-slate-400 hover:text-slate-600"}`}
                >
              ...
              <div className="p-8 space-y-8">
                ...
                <div className="space-y-3">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border-2 border-slate-50 rounded-xl px-5 py-3.5 focus:bg-white focus:border-emerald-500/20 outline-none transition-all font-bold text-sm shadow-inner"
                  />
                </div>
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                    Chọn Chuyên ngành <span className="text-red-500">*</span>
                  </label>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-3">
                    ...
                    <select
                      ...
                      className="w-full bg-slate-50 border-2 border-slate-50 rounded-xl px-4 py-3 focus:bg-white focus:border-emerald-500/20 outline-none cursor-pointer font-bold text-xs shadow-inner appearance-none"
                    >
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      {[
                        { _id: "Năm 1", name: "Năm 1" },
                        { _id: "Năm 2", name: "Năm 2" },
                        { _id: "Năm 3", name: "Năm 3" },
                        { _id: "Năm 4", name: "Năm 4" },
                        { _id: "Khác", name: "Khác" },
                      ].map((opt) => (
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-xl border-2 border-slate-50 focus-within:bg-white focus-within:border-emerald-500/20 transition-all shadow-inner relative">
                    {selectedTags.map((tag) => (
                      <span
                        key={tag._id}
                        className="bg-slate-900 text-white px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-wider flex items-center gap-2"
                      >
                        {tag.name}
                        <button
                          type="button"
                      ...
                      onKeyDown={(e) => {
                        if (e.key === "Enter" && tagInput.trim()) {
                          ...
                          if (existing) addTag(existing);
                            ...
                            setSelectedTags([
                              ...selectedTags,
                              { _id: tagInput.trim(), name: tagInput.trim() },
                            ]);
                          setTagInput("");
                        }
                      }}
                      placeholder="Thêm tag..."
                      className="flex-1 bg-transparent border-none outline-none font-bold text-xs min-w-[100px]"
                    />

                    {showTagSuggestions && tagInput.trim() !== "" && (
                      <div className="absolute top-full left-0 right-0 z-50 mt-2 bg-white rounded-xl shadow-2xl border border-slate-100 p-2 flex flex-row overflow-x-auto gap-2 [&::-webkit-scrollbar]:h-1.5 [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-track]:bg-transparent">
                        {allTags.filter((t) =>
                          ...
                          allTags
                            ...
                            .map((tag) => (
                              <button
                                ...
                                type="button"
                                onClick={() => addTag(tag)}
                                className="shrink-0 px-4 py-2 bg-slate-50 hover:bg-emerald-50 hover:text-emerald-600 border border-transparent hover:border-emerald-200 rounded-lg text-[10px] font-black uppercase text-slate-600 transition-all"
                              >
                                {tag.name}
                              </button>
                            ))
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {suggestedTags.map((t) => (
                      <button
                        ...
                        className="text-[9px] font-black text-emerald-600 bg-emerald-50 px-3 py-1 rounded-lg hover:bg-emerald-100 transition-all uppercase tracking-tighter"
                      >
                        #{t.name}
                      </button>
                    ))}
                ...
                <div className="grid grid-cols-1 md:grid-cols-5 gap-8 pt-6 border-t border-slate-100">
                  <div className="md:col-span-3 space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                      Nội dung tải lên <span className="text-red-500">*</span>
                    </label>
                    {uploadType === "file" ? (
                      <div
                        className={`relative rounded-2xl border-2 border-dashed h-40 flex flex-col items-center justify-center text-center p-4 transition-all cursor-pointer ${dragActive ? "border-emerald-500 bg-emerald-50/30" : "border-slate-100 bg-slate-50/50 hover:bg-slate-50"}`}
                      ...
                      >
                        {!file ? (
                          <div className="space-y-2">
                            <div className="w-10 h-10 rounded-xl bg-white shadow-sm flex items-center justify-center text-emerald-500 mx-auto border border-slate-50">
                              <svg
                                width="20"
                                height="20"
                                fill="none"
                          ...
                          <div className="flex items-center gap-3 p-3 bg-white rounded-xl border border-emerald-500/20 shadow-sm w-full">
                            ...
                            <p className="flex-1 text-[10px] font-black truncate uppercase text-slate-700">
                              {file.name}
                            </p>
                            <button
                              type="button"
                      ...
                      <div className="space-y-4">
                        <input
                          ...
                          className="w-full bg-slate-50 border-2 border-slate-50 rounded-xl px-4 py-3 focus:bg-white focus:border-emerald-500/20 outline-none font-bold text-xs shadow-inner"
                        />
                        {youtubePreview && (
                          <div className="relative aspect-video rounded-xl overflow-hidden shadow-lg border-2 border-white animate-in zoom-in-95">
                            ...
                            <div className="absolute inset-0 bg-black/10 flex items-center justify-center">
                              <div className="w-10 h-10 rounded-full bg-white/90 flex items-center justify-center text-red-600">
                                <svg
                                  width="20"
                                  height="20"
                                  fill="currentColor"
                  ...
                  <div className="md:col-span-2 space-y-4">
                    ...
                    <textarea
                      rows={uploadType === "link" && youtubePreview ? 8 : 6}
                      value={description}
                      onChange={(e) => setDescription(e.target.value)}
                      placeholder="Giới thiệu nhanh tài liệu..."
                      className="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl px-5 py-4 focus:bg-white focus:border-emerald-500/20 outline-none transition-all resize-none font-bold text-xs shadow-inner"
                    ></textarea>
                  </div>
                </div>
              </div>
              ...
              <div className="px-8 py-5 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">
                    Dữ liệu được bảo vệ an toàn trên hệ thống
                  </span>
                </div>
                <button
                  type="submit"
              ...
              <div className="bg-white rounded-[2rem] p-8 border border-slate-100 shadow-lg shadow-slate-200/30">
                ...
                <div className="space-y-6">
                  {[
                    {
                      t: "Đúng ngành",
                      d: "Chọn đúng ngành và chuyên mục để SV khác dễ tìm.",
                    },
                    {
                      t: "Bản quyền",
                      d: "Không đăng tài liệu mật hoặc vi phạm bản quyền.",
                    },
                    {
                      t: "Định dạng",
                      d: "Khuyên dùng PDF hoặc Link Video chuẩn.",
                    },
                  ].map((item, idx) => (
                    <div key={idx} className="flex gap-4">
                      <div className="shrink-0 w-1.5 h-1.5 rounded-full bg-emerald-500 mt-1" />
                      <div>
                        <p className="font-black text-[10px] text-slate-800 uppercase mb-1">
                          {item.t}
                        </p>
