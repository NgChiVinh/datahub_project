...
export default function Header() {
  ...
  // Các trang cần Header tối giản (chỉ hiện Logo)
  ...
  // Giao diện Header cho trang Login/Register
  if (isAuthPage) {
    return (
      <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-slate-50">
        <div className="container mx-auto flex h-20 items-center justify-center px-4">
          <Link href="/" className="flex items-center group">
            <div className="relative h-14 w-40 overflow-hidden group-hover:scale-105 transition-transform">
              <Image 
  ...
  // Giao diện Header mặc định cho các trang khác
  return (
    <header className="sticky top-0 z-50 w-full border-b border-slate-100 bg-white">
      <div className="container mx-auto flex h-20 items-center justify-between px-4 lg:px-8">
        ...
        <div className="flex items-center gap-6">
          <Link href="/upload" className="flex items-center gap-2 rounded-xl bg-primary px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-primary/20 transition-all hover:scale-105 hover:shadow-primary/30 active:scale-95 uppercase tracking-wider">
          ...
          <div className="relative" ref={dropdownRef}>
            <button 
              ...
              className="h-10 w-10 overflow-hidden rounded-full border-2 border-slate-100 bg-white p-0.5 shadow-md cursor-pointer hover:border-primary transition-all active:scale-90 flex items-center justify-center"
            >
              <div className="h-full w-full rounded-full bg-slate-50 flex items-center justify-center text-slate-600 text-xs font-bold uppercase">
                {session?.user ? (
